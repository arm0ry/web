...
const NavBarItem = ({ title, to, classprops }) => (
  <li>
    <Link to={to} className={`text-gray-700 hover:bg-gray-50 border-b border-gray-100 md:hover:bg-transparent md:border-0 block pl-3 pr-4 py-2 md:hover:text-blue-700 md:py-auto  md:px-0 ${classprops}`} >
      {title}
    </Link>
  </li>
);
const NavbarDropdownItem = ({ title, to, classprops, link }) => (
  <li>
    <Link to={to} className={`text-sm hover:bg-gray-100 text-gray-700 block px-4 py-2 ${classprops}`}>{title}</Link>
  </li>
);
  ...
  return (
    <>
      <li className="relative aligin">
        <button onClick={() => setTaggle((t) => !t)} onBlur={() => setTaggle((t) => false)} className="text-gray-700 hover:bg-gray-50 border-b border-gray-100 md:hover:bg-transparent md:border-0 pl-3 pr-4 py-2 md:hover:text-blue-700 md:py-auto  md:px-0  font-medium flex items-center justify-between w-full md:w-auto">
          {title}
          <svg fill="currentColor" viewBox="0 0 20 20" className={`inline w-4 h-4 mt-1 ml-1  transition-transform duration-200 transform md:-mt-1  ${taggle ? "rotate-180" : "rotate-0"}`}>
  ...
  <defs>
    <filter id="a" width="106" height="26.017" x=".5" y="2.484" color-interpolation-filters="sRGB" filterUnits="userSpaceOnUse">
      ...
      <feGaussianBlur result="effect1_foregroundBlur_18_737" stdDeviation=".5"/>
        ...
        <div className={`md:block w-full md:w-auto  my-auto ${!toggleMenu && "hidden"}`} >
          <ul className="flex-col md:flex-row flex md:space-x-8 mt-4 md:mt-0 md:text-sm md:font-medium">
            ...
            {walletAddress ?(<></>) :(<li>
              <CustomButton title="connect wallet" handleClick={() => { }} restStyles="rounded-full bg-yellow-200 ml-5 hidden md:block" />
            </li>) }
